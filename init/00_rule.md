...
3. 공통 개발 규칙
   ...
   3-10. 모든 개발 작업에 대해서는 json 파일 등 주석을 작성할 수 없는 환경을 제외하면 파일 내의 매 코드에 대해 한국어로 상세하고 이해하기 쉽게 작성한다. 기존에 작성된 주석의 경우에도 해당 주석이 가리키는 코드를 제거하게 된 경우에만 제거하도록 하고 그 외에는 유지하도록 한다. 함수를 작성하게 될 때에도 각 파라미터 및 리턴 값에 대한 데이터 타입과 반환되는 값에 대한 상세한 설명을 작성하도록 한다.
