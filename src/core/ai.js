...
const CONVENTIONAL_COMMIT_PATTERN =
  /^(feat|fix|refactor|docs|style|test|chore)(\([^)]+\))?!?:\s+\S/;
 ...
 * @param {string} params.mode - 실행 모드 ('step', 'batch', 'group')
...
export async function generateLargeDiffCommitMessage({
  ...
  if (!largeDiff.isLarge) {
    // prompt 초기화
    let prompt;
    // 그룹 모드이면서 그룹 정보가 있으면 그룹 커밋 메시지 생성 프롬프트 빌드
    if (mode === "group" && groupInfo) {
      prompt = buildGroupCommitPrompt({
        ...groupInfo,
        files,
        diff,
        language,
  ...
  // 아니라면 일반 커밋 메시지 생성 프롬프트 빌드
    ...
    prompt = buildSummaryCommitPrompt({
...
function selectCommitLine(value) {
  ...
  const conventionalLine = lines.find((line) =>
    CONVENTIONAL_COMMIT_PATTERN.test(line),
  );
