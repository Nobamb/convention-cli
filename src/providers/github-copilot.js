...
/**
 * GitHub Copilot API를 호출하여 AI 기반 커밋 메시지를 생성합니다.
 *
 * @param {object} params
...
export async function generateCommitMessage({ prompt, config = {}, headers = {} }) {
  ...
  const baseURL = config.baseURL ? config.baseURL.replace(/\/+$/u, "") : "https://api.githubcopilot.com/v1";
  const modelVersion = config.modelVersion || "copilot-codex";
  ...
  try {
    ...
    if (typeof message !== "string" || message.trim().length === 0) {
      throw new Error("GitHub-Copilot response did not include a commit message.");
    }
...
/**
 * GitHub Copilot API로부터 사용 가능한 모델 목록을 조회합니다.
 *
