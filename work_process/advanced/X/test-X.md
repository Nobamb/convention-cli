...
| Antigravity OAuth 설정 조회 | OAuth 지원 provider로 등록된 경우 `getOAuthProviderConfig("antigravity")` 호출 | provider별 URL과 scope가 분리된 설정으로 반환된다. |
...
| Antigravity style scope | `["https://api.antigravity.ai/auth/cli"]` | 검증 성공 |
